...
_openviking_codex_exec() {
  ...
  local _ov_url _ov_key _ov_account _ov_user _ov_agent
  if [ -f "$_ov_conf" ]; then
    local _ov_env
    ...
    _ov_env=$(node -e '
      try {
        ...
        process.stdout.write(
          ...
          out("OV_USER", c.user) +
          out("OV_AGENT", c.agent_id || c.agentId)
        );
      } catch {}
    ' "$_ov_conf" 2>/dev/null)
    eval "$_ov_env"
  ...
  _ov_agent="${OPENVIKING_AGENT_ID:-${OPENVIKING_AGENT:-${OV_AGENT:-}}}"
  unset OV_URL OV_KEY OV_ACCOUNT OV_USER OV_AGENT
  ...
  if command -v openviking-codex-plugin-render-cache >/dev/null 2>&1; then
    OPENVIKING_URL="$_ov_url" \
    OPENVIKING_API_KEY="$_ov_key" \
    OPENVIKING_ACCOUNT="$_ov_account" \
    OPENVIKING_USER="$_ov_user" \
    OPENVIKING_AGENT_ID="$_ov_agent" \
      openviking-codex-plugin-render-cache >/dev/null 2>&1 || true
  else
  ...
  [ -n "$_ov_agent" ]   && _env_args+=("OPENVIKING_AGENT_ID=$_ov_agent")
