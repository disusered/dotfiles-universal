...
async function main() {
  ...
  log("start", { sessionId, transcriptPath, trigger, scope: scopeConfig.activeScope });

  const state = await loadState(sessionId);
  state.openvikingScope = state.openvikingScope || scopeConfig.writeScope;
  activeFetchScope = state.openvikingScope;
  log("scope", { sessionId, scope: scopeLabel(activeFetchScope) });

  const health = await fetchJSON("/health", {}, activeFetchScope);
  if (!health) {
