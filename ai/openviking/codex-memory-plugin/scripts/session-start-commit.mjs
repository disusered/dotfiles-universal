...
async function commitOvSession(ovSessionId, scope = activeFetchScope) {
...
async function commitAndClear(state, reason) {
  if (state.ovSessionId) {
    ...
    const scope = state.openvikingScope || activeFetchScope;
    const commit = await commitOvSession(state.ovSessionId, scope);
    if (!commit) {
