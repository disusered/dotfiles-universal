#!/usr/bin/env bash
# Package the shared-bundle skill for upload to the Iteramind organization, alongside the
...
# skill at ai/agent-skills/okf-shared-bundle, or the team's copy drifts from ours.
set -euo pipefail

script_dir="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
canonical_skill="${script_dir}/../agent-skills/okf-shared-bundle"
artifact_dir="${script_dir}/dist"
artifact="${artifact_dir}/okf-shared-bundle.zip"
stage="$(mktemp -d)"
trap 'rm -rf -- "${stage}"' EXIT

mkdir -p -- "${artifact_dir}" "${stage}/.claude-plugin" "${stage}/skills"
cp -RL -- "${canonical_skill}" "${stage}/skills/okf-shared-bundle"
...
Pair it with the OKF connector. The connector provides the `okf_v1_*` tools; without this skill a
reader gets the tools and none of the rules.

It declares no MCP server and names no local path. The shared Bundle is objects in R2 reached
...
`ai/agent-skills/okf-shared-bundle`, then re-upload. The organization copy is a snapshot and
